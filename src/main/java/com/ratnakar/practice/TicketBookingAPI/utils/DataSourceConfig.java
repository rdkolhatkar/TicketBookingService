...
import org.springframework.beans.factory.annotation.Qualifier;  // For qualifying beans when multiple of same type
import org.springframework.beans.factory.annotation.Value;    // For injecting values from properties files
import org.springframework.context.annotation.Bean;          // For declaring Spring beans
import org.springframework.context.annotation.Configuration; // Marks class as configuration source
import org.springframework.orm.jpa.JpaTransactionManager;    // For JPA transaction management
import org.springframework.orm.jpa.LocalContainerEntityManagerFactoryBean; // For creating EntityManagerFactory
import org.springframework.orm.jpa.vendor.HibernateJpaVendorAdapter; // For Hibernate as JPA provider

// JDBC and DataSource imports
import javax.sql.DataSource;  // Standard Java interface for database connections
import java.util.HashMap;     // For mapping data source targets
import java.util.Map;         // For mapping data source targets

// HikariCP connection pool library (high-performance JDBC connection pool)
